...
class Attention(nn.Module):
    ...
    def forward(self, x,):
        ...
        # else:
        attn = (q @ k.transpose(-2, -1)) * self.scale  #4*12*785*785
        attn = attn.softmax(dim=-1)
        #weights = attn if self.vis else None
        weights = attn
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)  ##4*785*768
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, weights
...
class Block(nn.Module):
    ...
    def forward(self, x,):  #4*785*768
        ...
        x = x + self.drop_path(o)  #4*785*768
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
        img_size = to_2tuple(img_size)  #转换为元组类型
...
class VisionTransformer(nn.Module):
    ...
    def prepare_tokens(self, x):
        ...
        x = self.patch_embed(x)  # patch linear embedding  4*784*768
        ...
        #1*768*28*28 双三次插值法
        patch_pos_embed = patch_pos_embed.reshape(1, -1, h*w).permute(0, 2, 1)  #1*784*768
        ...
        x = torch.cat((cls_tokens, x), dim=1)  #4*785*768

        # add positional encoding to each token
        x = x + pos_embed   #4*785*768
    ...
    def forward_features(self, x):
        ...
        x = self.prepare_tokens(x)   #4*785*768

        x = self.pos_drop(x)  #4*785*768
        embeds = []
        atten = []
        ...
        atten.append(weights)

        x = self.norm(x)
        embeds[-1] = x
        return x[:, 0], x[:, 1:], embeds[self.aux_layer][:, 1:], atten
...
def vit_base_patch16_224(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg(mean = (0.5, 0.5, 0.5), std = (0.5, 0.5, 0.5),)
    if pretrained:

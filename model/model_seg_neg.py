...
class network(nn.Module):
    ...
    def forward_proj(self, crops, n_iter=None):
        ...
        global_view = crops[:2]  #2*3*448*448
        local_view = crops[2:]   #2*3*96*96

        local_inputs = torch.cat(local_view, dim=0)  #16*3*96*96

        self._EMA_update_encoder_teacher(n_iter)

        global_output_t = self.encoder.forward_features(torch.cat(global_view, dim=0))[0].detach()   #4*768
        output_t = self.proj_head_t(global_output_t)  #4*1024

        global_output_s = self.encoder.forward_features(torch.cat(global_view, dim=0))[0]  #4*768
        local_output_s = self.encoder.forward_features(local_inputs)[0]  #16*768
        output_s = torch.cat((global_output_s, local_output_s), dim=0)  #20*768
        output_s = self.proj_head(output_s) #20*1024
    ...
    def forward(self, x, cam_only=False, crops=None, n_iter=None):
        ...
        if crops is not None:
            output_t, output_s = self.forward_proj(crops, n_iter)  #4*1024  20*1024

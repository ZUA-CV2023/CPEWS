...
parser.add_argument("--pro_iters", default=10000, type=int, help="warmup_iters")
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        #             neg_loss.append(loss_neg)
        #
        #             # temp_loss = hard_triplet_dig(region_feat, pos_feats[idx], neg_feats[idx])
        #             if len(cur_neg_feats_unshared[idx]) == 0:
        #                 continue
        #
        #             temp_loss = hard_triplet_dig(region_feat, pos_feats[idx], cur_neg_feats_unshared[idx])
        #
        #             triplet_loss.append(temp_loss)
        #
        # loss_pos = torch.stack(pos_loss).mean() if len(pos_loss) > 0 else 0
        # loss_neg = torch.stack(neg_loss).mean() if len(neg_loss) > 0 else 0
        # triplet_loss = torch.stack(triplet_loss).mean() if len(triplet_loss) > 0 else 0
        #
        # loss_RC = (loss_pos + loss_neg) * args.RC_weight
        ...
        # warmup
        if n_iter <= 2000:
            loss = loss_RC + loss_PR + 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + 0.0 * seg_loss + 0.0 * reg_loss
            # loss = loss_RC + 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + 0.0 * seg_loss + 0.0 * reg_loss
        else:
            loss = loss_RC + loss_PR + 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_seg * seg_loss + args.w_reg * reg_loss
            # loss = loss_RC + triplet_loss + 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_seg * seg_loss + args.w_reg * reg_loss
        ...
        if (n_iter + 1) % args.log_iters == 0:
            ...
            if args.local_rank == 0:
                # logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; RC_loss: %.4f, cls_loss: %.4f, cls_loss_aux: %.4f, ptc_loss: %.4f, seg_loss: %.4f..." % (n_iter + 1, delta, eta, cur_lr, avg_meter.pop('RC_loss'), avg_meter.pop('cls_loss'), avg_meter.pop('cls_loss_aux'), avg_meter.pop('ptc_loss'), avg_meter.pop('seg_loss')))
                logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; RC_loss: %.4f, PR_loss: %.4f, cls_loss: %.4f, cls_loss_aux: %.4f, ptc_loss: %.4f, seg_loss: %.4f..." % (n_iter + 1, delta, eta, cur_lr, avg_meter.pop('RC_loss'), avg_meter.pop('PR_loss'), avg_meter.pop('cls_loss'), avg_meter.pop('cls_loss_aux'), avg_meter.pop('ptc_loss'), avg_meter.pop('seg_loss')))
...
if __name__ == "__main__":
    ...
    if args.local_rank == 0:
        ...
        setup_logger(filename=os.path.join(args.work_dir, 'train_vit_pro_4bs_4w_work8_vit.log'))

...
cls_labels_dict = np.load('../datasets/coco/cls_labels_onehot.npy', allow_pickle=True).item()
...
class VOC12ClassificationDatasetMSF(VOC12ClassificationDataset):
    ...
    def __getitem__(self, idx):
        name = self.img_name_list[idx]
        # name_str = decode_int_filename(name)  #图片名称(字符类型)

        # img = imageio.imread(get_img_path(name, self.voc12_root)) #返回图片路径名并读取图片

        img_name = get_img_path(name, self.voc12_root)
        img = np.asarray(robust_read_image(img_name))
        ...
        for s in self.scales:
            if s == 1:
                ...
                s_img = imutils.pil_rescale(img, s, order=3)  #对图像进行缩放
            s_img = self.img_normal(s_img) #333*500*3
            s_img = imutils.HWC_to_CHW(s_img)  #3*333*500
            ms_img_list.append(np.stack([s_img, np.flip(s_img, -1)], axis=0))#2*3*333*500 2*3*166*250 2*3*500*750 2*3*666*1000

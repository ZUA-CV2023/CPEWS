...
def _get_cluster(model, num_cls, region_thresh, vis_path, num_cluster, single_stage=False, mask_img=False, sampling_ratio=1.):
    trainmsf_dataset = datasets.dataloader.VOC12ClassificationDatasetMSF("../datasets/voc/train_aug.txt",
                                                                         voc12_root="../datasets/voc/VOCdevkit/VOC2012",
    ...
    with torch.no_grad():
        ...
        for img_idx, pack in enumerate(trainmsf_dataloader):
            ...
            for idx, img in enumerate(ms_imgs):
                cam, feat, _ = model.forward_feat(img[0].cuda(non_blocking=True)) #2*2048*18*32 2*21*18*32
            ...
            orig_img = cv2.imread('../datasets/voc/VOCdevkit/VOC2012/JPEGImages/{}.jpg'.format(name))  # 281*500*3
        ...
        for cls_idx in range(num_cls):
            ...
            for item_idx in top_prob_idx:
                ...
                for idx, img in enumerate(ms_imgs):
                    ...
                    if idx == 0:
                        alpha = 0.6
                        orig_img = cv2.imread('../datasets/voc/VOCdevkit/VOC2012/JPEGImages/{}.jpg'.format(pack['name']))

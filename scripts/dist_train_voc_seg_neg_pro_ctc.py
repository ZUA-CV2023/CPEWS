...
from model.losses_att import get_masked_ptc_loss, get_seg_loss, get_aff_loss, DenseEnergyLoss, get_energy_loss, CTCLoss_neg
from model.model_seg_neg_por_ctc import network
...
from utils.camutils_att import cam_to_label, get_mask_by_radius, cams_to_affinity_label,multi_scale_cam2, label_to_aff_mask, refine_cams_with_bkg_v2, crop_from_roi_neg, cam_to_roi_mask2
from utils.pyutils import AverageMeter, cal_eta, format_tabs, setup_logger

from step.cluster_utils_vit import get_regional_cluster
import importlib
...
parser.add_argument("--data_folder", default='../datasets/voc/VOCdevkit/VOC2012', type=str, help="dataset folder")
...
parser.add_argument("--max_iters", default=60000, type=int, help="max training iters")
...
parser.add_argument("--w_aff", default=0.1, type=float, help="w_reg")
...
parser.add_argument("--cam_num_epoches", default=1, type=int)
...
def train(args=None):
    ...
    CTC_loss = CTCLoss_neg(ncrops=10, temp=args.temp).cuda()
    ...
    for n_iter in range(args.max_iters):
        ...
        cls_label = cls_label.to(device, non_blocking=True)  #2*20
        ...
        cams, cams_aux = multi_scale_cam2(model, inputs=inputs, scales=args.cam_scales)  #2*20*448*448

        # get local crops from uncertain regions
        roi_mask = cam_to_roi_mask2(cams_aux.detach(), cls_label=cls_label, low_thre=args.low_thre,
                                    hig_thre=args.high_thre)

        local_crops, flags = crop_from_roi_neg(images=crops[2], roi_mask=roi_mask, crop_num=8,
        ...
        # cls, segs, fmap, cls_aux, atten_pred = model(inputs)
        #2*20 2*21*28*28 2*768*28*28 2*784*784
        ...
        # cls_loss = F.binary_cross_entropy_with_logits(logits, cls_label, reduction='mean')
        ...
        # attn_mask = get_mask_by_radius(h=28, w=28, radius=8)
        # aff_label = cams_to_affinity_label(refined_pseudo_label, mask=attn_mask, ignore_index=255)  # 2*400*400
        # aff_loss, pos_count, neg_count = get_aff_loss(atten_pred, aff_label)

        # ptc loss
        resized_cams_aux = F.interpolate(cams_aux, size=fmap.shape[2:], mode="bilinear", align_corners=False)  #2*20*28*28
        ...
        # warmup
        if n_iter <= 2000:
            loss = loss_RC + 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_ctc * ctc_loss + 0.0 * seg_loss + 0.0 * reg_loss
        else:
            loss = loss_RC + 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss+ args.w_ctc * ctc_loss  + args.w_seg * seg_loss + args.w_reg * reg_loss
        ...
        if (n_iter + 1) % args.log_iters == 0:
            ...
            if args.local_rank == 0:
                logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; RC_loss: %.4f, cls_loss: %.4f, cls_loss_aux: %.4f, ptc_loss: %.4f, ctc_loss: %.4f, seg_loss: %.4f..." % (n_iter + 1, delta, eta, cur_lr, avg_meter.pop('RC_loss'), avg_meter.pop('cls_loss'), avg_meter.pop('cls_loss_aux'), avg_meter.pop('ptc_loss'), avg_meter.pop('ctc_loss'), avg_meter.pop('seg_loss')))
...
if __name__ == "__main__":
    ...
    if args.local_rank == 0:
        ...
        setup_logger(filename=os.path.join(args.work_dir, 'train_vit_pro_ctc.log'))

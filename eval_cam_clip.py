...
def run_eval_cam(args, print_log=False, is_coco=False):
    ...
    labels = []
    n_images = 0
    for i, id in enumerate(tqdm(eval_list)):
        n_images += 1
        if args.cam_type == 'png':
            ...
            cams = cam_dict['msc_seg'] #2*281*500


            # import matplotlib.pyplot as plt
            # def visualize_cam(cam_array):
            #     num_classes, H, W = cam_array.shape
            #
            ...
            #         else:
            #             ax = axes
            #         cax = ax.imshow(cam, cmap='jet')
            #         ax.axis('off')
            #         ax.set_title(f'Class {i + 1} CAM')
            #
            #         # 添加颜色条
            ...
            #
            # # 可视化 CAM
            # visualize_cam(cams[0])

    #         if 'bg' not in args.cam_type:
    #             if args.cam_eval_thres < 1:
    #                 cams = np.pad(cams, ((1, 0), (0, 0), (0, 0)), mode='constant', constant_values=args.cam_eval_thres)
    #             else:
    #                 bg_score = np.power(1 - np.max(cams, axis=0, keepdims=True), args.cam_eval_thres) #1*281*500
    #                 cams = np.concatenate((bg_score, cams), axis=0) #2*281*500
    #         # keys = np.pad(cam_dict['msc_seg'][0] + 1, (1, 0), mode='constant')
    #         keys = cam_dict['msc_seg'][0]
            cls_labels = np.argmax(cams[0], axis=0)  #281*500
    #
            import matplotlib.pyplot as plt
            def visualize_cam(cam_tensor):

                # 标准化 CAM 到 [0, 1] 区间
                cam_array = (cam_tensor - np.min(cam_tensor)) / (np.max(cam_tensor) - np.min(cam_tensor))
                ...
                out_im = os.path.join(output, f'{id}.png')
                plt.savefig(out_im)
                plt.show()


            # 可视化 CAM
            visualize_cam(cls_labels)
    #
    #         cls_labels = keys[cls_labels].astype(np.uint8) #281*500
    #         # cls_labels[cls_labels == 0] = 0
    #         # cls_labels[cls_labels != 0] = 255
    #         # # 将label转换为RGB图像
    #         # label_rgb = np.stack((cls_labels, cls_labels, cls_labels), axis=2)
    #         # # 保存为图像文件
    #         # label_image = Image.fromarray(label_rgb)
    #         # label_image.save('your_label.png')
    #     preds.append(cls_labels)
    #     gt_file = os.path.join(args.gt_root, '%s.png' % id)
    #     gt = np.array(Image.open(gt_file)).astype(np.uint8)
    #     labels.append(gt) #281*500
    #
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cam_out_dir", default="F:/ToCo-main/scripts/work_dir_voc_wseg/2024-07-11-08-59-27-750424/segs/logits/val", type=str)
    parser.add_argument("--cam_type", default="bg", type=str)
    # parser.add_argument("--split_file", default="/home/xxx/datasets/VOC2012/ImageSets/Segmentation/train.txt", type=str)
    parser.add_argument("--split_file", default="F:/ToCo-main/datasets/voc/val.txt", type=str)
    parser.add_argument("--cam_eval_thres", default=2, type=float)
    parser.add_argument("--gt_root", default="./VOCdevkit2012/VOC2012/SegmentationClassAug", type=str)

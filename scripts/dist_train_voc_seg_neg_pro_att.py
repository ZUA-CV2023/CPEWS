...
from model.losses_att import get_masked_ptc_loss, get_seg_loss, get_aff_loss, DenseEnergyLoss, get_energy_loss
from model.model_seg_neg_por_att import network
...
from utils.camutils_att import cam_to_label, get_mask_by_radius, cams_to_affinity_label,multi_scale_cam2, label_to_aff_mask, refine_cams_with_bkg_v2, crop_from_roi_neg
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        # warmup
        if n_iter <= 2000:
            loss = loss_RC + loss_PR + 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + 0.0 * seg_loss + 0.0 * reg_loss  + args.w_aff * aff_loss
        else:
            loss = loss_RC + loss_PR + 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_ptc * ptc_loss + args.w_seg * seg_loss + args.w_reg * reg_loss + args.w_aff * aff_loss
        ...
        if (n_iter + 1) % args.log_iters == 0:
            ...
            if args.local_rank == 0:
                logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; RC_loss: %.4f, PR_loss: %.4f, cls_loss: %.4f, cls_loss_aux: %.4f, ptc_loss: %.4f, seg_loss: %.4f, aff_loss: %.4f..." % (n_iter + 1, delta, eta, cur_lr, avg_meter.pop('RC_loss'), avg_meter.pop('PR_loss'), avg_meter.pop('cls_loss'), avg_meter.pop('cls_loss_aux'), avg_meter.pop('ptc_loss'), avg_meter.pop('seg_loss'), avg_meter.pop('aff_loss')))
...
if __name__ == "__main__":
    ...
    if args.local_rank == 0:
        ...
        setup_logger(filename=os.path.join(args.work_dir, 'train_vit_pro_aff.log'))

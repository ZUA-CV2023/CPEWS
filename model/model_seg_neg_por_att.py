...
# from model.backbone import vit_att as encoder
from . import decoder
"""
...
class network(nn.Module):
    ...
    def forward(self, x, cam_only=False):
        ...
        _, _x, x_aux, atten = self.encoder.forward_features(x)
        ...
        atten = atten + atten.permute(0, 1, 3, 2)  # A：预测的语义亲和力矩阵
        atten_pred = self.attn_proj(atten)  # 2*1*400*400 #将输入的特征进行线性变换  目的是增强特征的空间关联性
        attn_pred = torch.sigmoid(atten_pred)[:, 0, ...]  # 预测的语义亲和力矩阵权重2*400*400
    ...
    def forward_feat(self, x, cam_only=False, crops=None, n_iter=None):
        ...
        cls_token, _x, x_aux, _ = self.encoder.forward_features(x)

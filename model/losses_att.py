...
def get_aff_loss(inputs, targets):  #2*400*400
...
def get_masked_ptc_loss(inputs, mask):
    ...
    def cos_sim(x):   #2*768*784
        x = F.normalize(x, p=2, dim=1, eps=1e-8)   #2*768*784
        cos_sim = torch.matmul(x.transpose(1,2), x)  #余弦相似矩阵 2*784*784

        # import matplotlib.pyplot as plt
        #
        # tensor_np = cos_sim.detach().cpu().numpy()
        #
        # for i in range(tensor_np.shape[0]):
        #     plt.subplot(1, 2, i + 1)  # 创建1行2列的子图
        #     plt.imshow(tensor_np[i], cmap='viridis', interpolation='nearest')  # 绘制热力图
        #     plt.title(f'Layer {i + 1}')  # 设置标题
        #     plt.colorbar()  # 添加颜色条
        #
        # plt.suptitle('Similarity Maps for Layers 1 and 2')  # 设置总标题
        # plt.savefig('similarity_maps.png')  # 保存图像
        # plt.show()  # 显示图像
...
def get_energy_loss(img, logit, label, img_box, loss_layer, mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375]):
    ...
    pred_prob = F.softmax(logit, dim=1)  #2*21*448*448
    crop_mask = torch.zeros_like(pred_prob[:,0,...])  #2*448*448

    for idx, coord in enumerate(img_box):
        crop_mask[idx, coord[0]:coord[1], coord[2]:coord[3]] = 1

    _img = torch.zeros_like(img)  #2*3*448*448
    _img[:,0,:,:] = img[:,0,:,:] * std[0] + mean[0]   #归一化
...
class CTCLoss_neg(nn.Module):
    ...
    def forward(self, student_output, teacher_output, flags):
        ...
        student_out = student_output.reshape(self.ncrops, b, -1).permute(1,0,2)  #2*10*1024
        teacher_out = teacher_output.reshape(2, b, -1).permute(1,0,2)   #2*2*1024

        logits = torch.matmul(teacher_out, student_out.permute(0,2,1))  #2*2*10  相似矩阵
        logits = torch.exp(logits / self.temp)  #2*2*10

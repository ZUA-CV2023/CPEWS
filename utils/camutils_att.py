...
def cam_to_roi_mask2(cam, cls_label, hig_thre=None, low_thre=None):
    b, c, h, w = cam.shape
    #pseudo_label = torch.zeros((b,h,w))
    cls_label_rep = cls_label.unsqueeze(-1).unsqueeze(-1).repeat([1,1,h,w])  #2*20*448*448
    valid_cam = cls_label_rep * cam  #2*20*448*448
    cam_value, _ = valid_cam.max(dim=1, keepdim=False)  #2*448*448
    # _pseudo_label += 1
    roi_mask = torch.ones_like(cam_value, dtype=torch.int16)  #2*448*448
    roi_mask[cam_value<=low_thre] = 0
    roi_mask[cam_value>=hig_thre] = 2  #2*448*448
...
def crop_from_roi_neg(images, roi_mask=None, crop_num=8, crop_size=96):
    ...
    _crops = torch.chunk(temp_crops, chunks=crop_num, dim=1,)  #分割成八块
    crops = [c[:, 0] for c in _crops]  #2*3*96*96

    return crops, flags

def multi_scale_cam2(model, inputs, scales):  #2*3*448*448
    '''process cam and aux-cam'''
    # cam_list, tscam_list = [], []
    b, c, h, w = inputs.shape
    with torch.no_grad():
        inputs_cat = torch.cat([inputs, inputs.flip(-1)], dim=0) #4*3*448*448

        _cam_aux, _cam = model(inputs_cat, cam_only=True)  #4*20*28*28

        _cam = F.interpolate(_cam, size=(h,w), mode='bilinear', align_corners=False)
        _cam = torch.max(_cam[:b,...], _cam[b:,...].flip(-1))  #2*20*448*448
        _cam_aux = F.interpolate(_cam_aux, size=(h,w), mode='bilinear', align_corners=False)
        _cam_aux = torch.max(_cam_aux[:b,...], _cam_aux[b:,...].flip(-1))  #2*20*448*448
        ...
        cam = torch.sum(torch.stack(cam_list, dim=0), dim=0)  #2*20*448*448
        cam = cam + F.adaptive_max_pool2d(-cam, (1, 1))  #2*20*448*448
...
def label_to_aff_mask(cam_label, ignore_index=255):
    ...
    b,h,w = cam_label.shape  #2*28*28

    _cam_label = cam_label.reshape(b, 1, -1)   #2*1*784
    _cam_label_rep = _cam_label.repeat([1, _cam_label.shape[-1], 1])  #2*784*784
    ...
    for i in range(b):
        ...
        aff_label[i, _cam_label_rep[i, 0, :]==ignore_index, :] = ignore_index
    aff_label[:, range(h*w), range(h*w)] = ignore_index  #对角线元素也为忽略值
    return aff_label
...
def refine_cams_with_bkg_v2(ref_mod=None, images=None, cams=None, cls_labels=None, high_thre=None, low_thre=None, ignore_index=False,  img_box=None, down_scale=2):
    ...
    b,_,h,w = images.shape  #2*3*448*448
    _images = F.interpolate(images, size=[h//down_scale, w//down_scale], mode="bilinear", align_corners=False)
    # 2*3*224*224

    bkg_h = torch.ones(size=(b,1,h,w))*high_thre  #2*1*448*448
    bkg_h = bkg_h.to(cams.device)
    bkg_l = torch.ones(size=(b,1,h,w))*low_thre   #2*1*448*448
    bkg_l = bkg_l.to(cams.device)

    bkg_cls = torch.ones(size=(b,1))   #2*1
    bkg_cls = bkg_cls.to(cams.device)
    cls_labels = torch.cat((bkg_cls, cls_labels), dim=1)  #2*21
    ...
    for idx, coord in enumerate(img_box):
        ...
        valid_cams_h = _cams_with_bkg_h[idx, valid_key, ...].unsqueeze(0).softmax(dim=1)  #1*2*224*224
...
def _refine_cams(ref_mod, images, cams, valid_key, orig_size):
    ...
    refined_cams = ref_mod(images, cams)   #1*2*224*224
    refined_cams = F.interpolate(refined_cams, size=orig_size, mode="bilinear", align_corners=False)  #1*2*448*448
    refined_label = refined_cams.argmax(dim=1)   #1*448*448
    refined_label = valid_key[refined_label]   #1*448*448
...
def cams_to_affinity_label(cam_label, mask=None, ignore_index=255):
    ...
    cam_label_resized = F.interpolate(cam_label.unsqueeze(1).type(torch.float32), size=[h // 16, w // 16],
                                      mode="nearest")  # 2*1*20*20

    _cam_label = cam_label_resized.reshape(b, 1, -1)  # 2*1*400
    _cam_label_rep = _cam_label.repeat([1, _cam_label.shape[-1], 1])  # 2*400*400
    _cam_label_rep_t = _cam_label_rep.permute(0, 2, 1)  # 2*400*400
    aff_label = (_cam_label_rep == _cam_label_rep_t).type(torch.long)  # 2*400*400
...
def get_mask_by_radius(h=28, w=28, radius=8):
    hw = h * w
